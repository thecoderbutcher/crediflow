...
import { createInstallments } from '@/app/(protected)/installments/create/action/create';
...
const FormCreateLoan = ({ loanType, paymentType }: FormCreateLoanProps) => {
  ...
  return (
    <form
      className="flex flex-col gap-6 px-4 py-8 bg-darkText shadow-md"
      onSubmit={handleSubmit(onSubmit)}
    >
      <FormGroup
        Icon={FaMoneyBillAlt}
        labelFor="typeinteres"
        labelText="Tipo de interes"
        errorMessage={errors.loanTypeId?.message}
      >
        <select
          {...register('loanTypeId')}
          name="loanTypeId"
      ...
      <FormGroup
        Icon={GiReceiveMoney}
        labelFor="totalInstallments"
        labelText="Cuotas"
        errorMessage={errors.totalInstallments?.message}
      >
        <input
      ...
      <FormGroup
        Icon={FaMoneyBillAlt}
        labelFor="paymentFrequencyId"
        labelText="Frecuencia del pago"
        errorMessage={errors.paymentFrequencyId?.message}
      >
        <select
      ...
      <div className="flex w-full items-center gap-2  shadow-md">
        <button
          type="submit"
          className="bg-primary text-darkText py-2 w-full text-center rounded-md"
        >
          Guardar
        </button>
      </div>
